def run_pipeline(portal):
  ...
  try:
    ...
    try:
        ...
        log.info("Connected to Figaro (plain FTP)")
        log.info(f"PWD: {ftp.pwd()}")
        log.info(f"FILES BEFORE: {ftp.nlst()}")

        with open(ZIP_NAME, "rb") as f:
            ftp.storbinary(f"STOR {ZIP_NAME}", f)

        log.info(f"FILES AFTER UPLOAD: {ftp.nlst()}")

        # Wait 5 seconds to see if server deletes it
        time.sleep(5)
        log.info(f"FILES 5 SECONDS LATER: {ftp.nlst()}")

        ftp.quit()

        log.info(f"✅ Upload attempt finished")
